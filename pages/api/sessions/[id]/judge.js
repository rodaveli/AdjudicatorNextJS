...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    try {
      ...
      const winning_argument = sessionArguments.find(
        (arg) => arg.user_id === judgement_data.winning_user_id,
      );
      const losing_argument = sessionArguments.find(
        (arg) => arg.user_id === judgement_data.losing_user_id,
      );

      judgement_data.winner = winning_argument
        ? winning_argument.username
        : "Unknown";
      judgement_data.loser = losing_argument
        ? losing_argument.username
        : "Unknown";
      ...
      // Send event for judgement ready
