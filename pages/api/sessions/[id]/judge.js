...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      arguments.sort((a, b) => a.user_id.localeCompare(b.user_id));

      const judgement_data = await get_ai_judgement(arguments);

      const winning_argument = arguments.find((arg) => arg.user_id === judgement_data.winning_user_id);
      const losing_argument = arguments.find((arg) => arg.user_id === judgement_data.losing_user_id);
