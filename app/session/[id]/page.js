...
export default function Session() {
  ...
  useEffect(() => {
    if (session && userId) {
      setCurrentUser(userId === session.user1_id ? 'user1' : userId === session.user2_id ? 'user2' : null);
    }
  }, [session, userId]);
  ...
  const addMessage = (content) => {
    setMessages((prevMessages) => [...prevMessages, { content, timestamp: new Date() }]);
  };

  const canSubmitArgument = session?.arguments && session.arguments.length < 2;
  const canGetJudgement = session?.arguments && session.arguments.length === 2 && !session?.judgement;
  const canAppeal = session?.judgement && session.judgement.loser === session[`${currentUser}_name`] && !session.appeal_judgement;
  ...
  return (
    <main>
      ...
      {session.appeal_judgement && <Judgement judgement={session.appeal_judgement} />}
